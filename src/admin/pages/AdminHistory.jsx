...
const getRazorpayPaymentId = (sessionOrders) => {
  return (
    sessionOrders.find((order) => order.razorpayPaymentId)?.razorpayPaymentId ||
    sessionOrders.find((order) => order.payment?.razorpayPaymentId)?.payment
      ?.razorpayPaymentId ||
    ""
  );
};
...
function OrderHistoryCard({
  ...
  const razorpayPaymentId = getRazorpayPaymentId(sessionOrders);
    ...
    <article className="flex flex-col overflow-hidden bg-white border border-gray-100 shadow-[0_8px_26px_rgba(15,23,42,0.08)] rounded-xl">
      ...
      <div className="px-5 py-3 border-t border-gray-100 bg-[#fffdfa]">
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          ...
          {razorpayPaymentId && (
            <div className="px-3 py-2 mt-3 rounded-xl bg-emerald-50">
