...
function CurrentTableCard({
  table,
  onCompleteCurrentToken,
  onExpireAllTableTokens,
  completingToken,
  completingTable,
}) {
  ...
  const currentTokenLoading = completingToken === session?.token;
  const tableTokensLoading = completingTable === table.tableNumber;
        ...
        {isActive ? (
          <>
            ...
            <div className="grid gap-3">
              <button
                type="button"
                disabled={currentTokenLoading || tableTokensLoading}
                onClick={() =>
                  onCompleteCurrentToken(session?.token, table.tableNumber)
                }
                className={`flex items-center justify-center w-full gap-2 py-3 text-sm font-extrabold text-white rounded-xl shadow-[0_8px_18px_rgba(7,24,50,0.18)] transition ${
                  currentTokenLoading || tableTokensLoading
                    ? "bg-slate-400 cursor-not-allowed"
                    : "bg-[#071832] hover:bg-[#0b254d] active:scale-[0.99]"
                }`}
              >
                {currentTokenLoading ? (
                  <>
              ...
              <button
                type="button"
                disabled={currentTokenLoading || tableTokensLoading}
                onClick={() => onExpireAllTableTokens(table.tableNumber)}
                className={`flex items-center justify-center w-full gap-2 py-3 text-sm font-extrabold rounded-xl border transition ${
                  currentTokenLoading || tableTokensLoading
                    ? "bg-slate-100 text-slate-400 border-slate-200 cursor-not-allowed"
                    : "bg-red-50 text-red-600 border-red-100 hover:bg-red-100 active:scale-[0.99]"
                }`}
              >
                {tableTokensLoading ? (
                  <>
                    <RefreshCw size={17} className="animate-spin" />
                    Expiring All Table Tokens...
                  </>
                ) : (
                  <>
                    <Trash2 size={17} />
                    Expire All Tokens of This Table
                  </>
                )}
              </button>
            </div>
...
export default function AdminCurrentTables() {
  ...
  const completeCurrentToken = (token, tableNumber) => {
    if (!token) {
    ...
    if (
      !window.confirm(`Complete current visible token of Table ${tableNumber}?`)
    ) {
      return;
    }
    ...
    api
      ...
      .then(() => {
        toast.success(`Current token completed for Table ${tableNumber}`);
        fetchCurrentTables(false);
      })
      .catch((err) => {
        console.log(err);
        ...
        toast.error(
          err.response?.data?.message ||
            ...
            "Failed to complete current token",
        );
      })
      .finally(() => {
  ...
  const expireAllTokensOfTable = (tableNumber) => {
    if (!tableNumber) {
    ...
    if (
      !window.confirm(
        `Expire every active token/session of Table ${tableNumber}?`,
      )
    ) {
      return;
    }
    ...
    api
      .put(`/table/complete-all/${tableNumber}`)
      .then((res) => {
        toast.success(
          res.data?.message ||
            `All active tokens expired for Table ${tableNumber}`,
        );
        fetchCurrentTables(false);
      })
      .catch((err) => {
        ...
        toast.error(
          err.response?.data?.message ||
            err.response?.data?.error ||
            "Failed to expire all tokens of this table",
        );
      })
      .finally(() => {
  ...
  const expireAllActiveSessions = () => {
    if (stats.active === 0) {
      toast.error("No active table sessions found");
      return;
    }

    if (!window.confirm("Expire every active token/session of every table?")) {
      return;
    }
    ...
    api
      .put("/tables/complete-all-active")
      .then((res) => {
        toast.success(res.data?.message || "All active sessions expired");
        fetchCurrentTables(false);
      })
      .catch((err) => {
        console.log(err);
        ...
        toast.error(
          err.response?.data?.message ||
            err.response?.data?.error ||
            "Failed to expire all active sessions",
        );
      })
      .finally(() => {
      ...
      <main className="px-4 py-7 mx-auto max-w-[1800px] sm:px-6">
        ...
        <div className="flex flex-col gap-5 mb-6 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
            <button
              type="button"
              onClick={() => fetchCurrentTables(false)}
              className="inline-flex items-center justify-center gap-2 px-5 py-3 text-sm font-extrabold text-slate-700 transition bg-white border border-gray-100 rounded-xl shadow-sm hover:bg-slate-50 active:scale-[0.99]"
            >
              <RefreshCw size={16} />
              Refresh
            </button>
            ...
            <button
              type="button"
              disabled={completingAll || stats.active === 0}
              onClick={expireAllActiveSessions}
              className={`inline-flex items-center justify-center gap-2 px-5 py-3 text-sm font-extrabold text-white transition rounded-xl shadow-[0_10px_24px_rgba(7,24,50,0.18)] ${
                completingAll || stats.active === 0
                  ? "bg-slate-400 cursor-not-allowed"
          ...
          <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">
            {tables.map((table) => (
              <CurrentTableCard
                ...
                table={table}
                onCompleteCurrentToken={completeCurrentToken}
                onExpireAllTableTokens={expireAllTokensOfTable}
                completingToken={completingToken}
                completingTable={completingTable}
              />
            ))}
          </div>
        ) : (
