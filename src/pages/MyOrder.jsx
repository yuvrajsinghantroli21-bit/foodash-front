...
export default function MyOrder() {
  ...
  useEffect(() => {
    ...
    const handleUpdate = (updatedOrder) => {
      if (
        updatedOrder?.token === token ||
        updatedOrder?.sessionId === token ||
        updatedOrder?.token === undefined
      ) {
        fetchSessionBill();
    ...
    socket.on("tables-current-updated", handleUpdate);
    ...
    return () => {
      ...
      socket.off("tables-current-updated", handleUpdate);
        ...
        <div className="mt-5 rounded-[1.7rem] border border-amber-100 bg-[#fffaf1] p-4">
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            <button
              type="button"
              disabled
              className="inline-flex items-center justify-center h-12 gap-2 px-5 text-sm font-black cursor-not-allowed rounded-2xl bg-slate-200 text-slate-500"
            >
              <CreditCard size={17} />
              Pay Online Soon
            </button>
            ...
            <button
              ...
              className="inline-flex h-12 items-center justify-center gap-2 rounded-2xl bg-[#3d2412] px-5 text-sm font-black text-amber-100 shadow-lg transition hover:-translate-y-0.5 hover:bg-[#2c190d] disabled:opacity-60"
            >
              {paymentLoading ? (
